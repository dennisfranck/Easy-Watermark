...
class PostType {
	...
	public function admin_notices() {
		...
		if ( 'watermark' === get_current_screen()->id && 999 <= $this->get_watermarks_count() && 'publish' !== $post->post_status ) {
	...
	public function post_row_actions( $actions, $post ) {
		if ( 'watermark' === $post->post_type ) {
			if ( 999 <= $this->get_watermarks_count() && isset( $actions['untrash'] ) ) {
	...
	public function get_watermarks_count() {
		$count = wp_count_posts( 'watermark' );
	...
	public function setup_columns( $columns ) {
		...
		if ( 999 <= $this->get_watermarks_count() && 'publish' !== $post->post_status ) {
			// Force one column.
			return 1;
		}
	...
	public function change_title_support() {
		...
		if ( 999 <= $this->get_watermarks_count() && isset( $_wp_post_type_features['watermark']['title'] ) ) {
	...
	public function pre_untrash_post( $untrash, $post ) {
		...
		if ( 'watermark' === $post->post_type && 999 <= $this->get_watermarks_count() ) {
			return true;
		}
	...
	public function wp_insert_post_data( $data, $postarr ) {
		...
		if ( 'watermark' === $data['post_type'] && isset( $postarr['watermark'] ) ) {
			...
			$new_attachment_id = isset( $watermark_data['attachment_id'] ) ? $watermark_data['attachment_id'] : null;
