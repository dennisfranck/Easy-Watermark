...
use EasyWatermark\Helpers\SvgConverter;
...
class AttachmentProcessorGD extends AttachmentProcessor {
	 ...
	 * @var array
	 */
	private $allowed_types = [ 'jpeg', 'png', 'gif', 'webp', 'avif' ];
	...
	/**
	 * Sets image file and resets internal state
	 *
	...
	public function set_file( $file ) {
		...
		// Reset properties to ensure the new image is loaded correctly
	...
	public function print_text_preview( $watermark, $format = 'png' ) {
		...
		// Set image size manually to prevent errors in apply_text_watermark
		$this->image_size = [
	...
	private function create_image( $file_path, $type ) {
		...
		if ( false === $image ) {
			...
			file_put_contents( $log_file, "GD ERROR: Failed to create image from $file_path using $func_name\n", FILE_APPEND );
			return false;
		}
	...
	private function apply_image_watermark( $watermark ) {
		...
		// Check for SVG and convert on-the-fly if needed
		$svg_png_path = SvgConverter::get_png_path( $watermark->attachment_id );
		if ( $svg_png_path ) {
			$watermark_file = $svg_png_path;
			$watermark->mime_type = 'image/png'; // Treat as PNG after conversion
		}
		...
		if ( $watermark->opacity < 100 ) {
			// Watermark has opacity, use custom merge function to preserve alpha channel.
			$func_name = [ $this, 'image_copy_merge_alpha' ];
			$params[]  = $watermark->opacity;
		} else {
			// Fully opaque watermark, use imagecopy.
			$func_name = 'imagecopy';
		}
	...
	private function detect_image_type( $type, $file_path = null ) {
		...
		if ( is_string( $type ) && 0 === strpos( $type, 'image/' ) ) {
			$type = substr( $type, 6 );
		}
	...
	private function prepare_output( $output = null ) {
		...
		if ( 'jpeg' === $this->image_type || 'webp' === $this->image_type || 'avif' === $this->image_type ) {
	...
	/**
	 * Merges two images with opacity preserving alpha channel
	 *
	 ...
	 * @return boolean
	 */
	public function image_copy_merge_alpha( $dst_im, $src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct ) {
		...
		$result = imagecopymerge( $dst_im, $cut, $dst_x, $dst_y, 0, 0, $src_w, $src_h, $pct );
